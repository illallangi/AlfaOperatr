...
class TemplateConsumer:
    def __init__(self, alfa_template, config, session=None, queue=None, logger=None):
        ...
        self.logger = Log.get_logger(f'TemplateConsumer({alfa_template["metadata"]["name"]})', self.config.log_level) if logger is None else logger
    ...
    async def consume(self):
        for render in await TemplateRenderer(recursive_get(self.alfa_template, "metadata.name"), self.config, self.session).render():
                ...
                async with self.session.request('get', url) as item_get_response:
                    if item_get_response.status in [404]:
                        ...
                        if "PersistentVolumeClaim" == item_get.get("kind"):
                            self.logger.info(f'Not updating as PersistentVolumeClaim is immutable after creation {render["kind"]} {url}')
                            continue
                        ...
                        if "Service" == item_get.get("kind") and "externalTrafficPolicy" in item_get.get("spec", {}) and "Local" == item_get.get("spec", {}).get("externalTrafficPolicy"):
                            render["spec"]["healthCheckNodePort"] = item_get["spec"]["healthCheckNodePort"]
