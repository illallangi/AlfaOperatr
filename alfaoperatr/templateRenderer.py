...
class TemplateRenderer:
    def __init__(self, name, config, session=None, jinja=None, logger=None):
        ...
        self.logger = Log.get_logger(f'TemplateRenderer({name})', self.config.log_level) if logger is None else logger

    async def render(self):
        self.logger.info(f'Rendering AlfaTemplate {self.name} in {await self.scope} scope')
    ...
    async def items(self):
        if '_items' not in self.__dict__ or self._items is None:
            ...
            self._items = {
                k: await self.get_items(k)
                for k
                in await self.kinds
            }
            for k in self._items:
                if self.config.debug_path:
                    with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-{k.lower()}s.yaml'), 'w') as outfile:
    ...
    async def objects(self):
        if '_objects' not in self.__dict__ or self._objects is None:
            ...
            self._objects = [
                {
                    ...
                    'metadata': {
                        'labels': {
                            (await self.labels_name): recursive_get(item, f'metadata#labels#{await self.labels_name}', sep='#') or (await self.parent_kind).lower(),
                            (await self.labels_instance): recursive_get(item, f'metadata#labels#{await self.labels_instance}', sep='#') or recursive_get(item, 'metadata.name', default=''),
                            (await self.labels_domain_name): recursive_get(item, f'metadata#labels#{await self.labels_domain_name}', sep='#') or recursive_get(item, 'spec.domainName', default=''),
                            (await self.labels_component): recursive_get(item, f'metadata#labels#{await self.labels_component}', sep='#') or await self.component or '',
                        ...
                        'ownerReferences': [
                            ...
                            for i
                            in (await self.items)[await self.parent_kind]
                            if (await self.owner_references) and recursive_get(i, 'metadata.uid') == recursive_get(item, 'metadata.uid')
                        ]
                    },
                    'selector': {
                        (await self.labels_name): recursive_get(item, f'metadata#labels#{await self.labels_name}', sep='#') or (await self.parent_kind).lower(),
            ...
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-objects.yaml'), 'w') as outfile:
    ...
    async def domains(self):
        if '_domains' not in self.__dict__ or self._domains is None:
                            ...
                            ]),
                        {
                            'metadata': {
                                ...
                                'ownerReferences': [
                                    ...
                                    for i
                                    in (await self.items)[await self.parent_kind]
                                    if (await self.owner_references) and recursive_get(i, 'spec.domainName') == d
                                ]
                            },
                            'selector': {
                                (await self.labels_instance): ''
            ...
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-domains.yaml'), 'w') as outfile:
    ...
    async def namespaces(self):
        if '_namespaces' not in self.__dict__ or self._namespaces is None:
                            ...
                            ]),
                        {
                            'metadata': {
                                ...
                                'ownerReferences': [
                                    ...
                                    for i
                                    in (await self.items)[await self.parent_kind]
                                    if (await self.owner_references) and recursive_get(i, 'metadata.namespace') == n
                                ]
                            },
                            'selector': {
                                (await self.labels_domain_name): ''
            ...
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-namespaces.yaml'), 'w') as outfile:
    ...
    async def clusters(self):
        if '_clusters' not in self.__dict__ or self._clusters is None:
            ...
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-clusters.yaml'), 'w') as outfile:
    ...
    async def renders(self):
        if '_renders' not in self.__dict__ or self._renders is None:
                        ...
                        'metadata': {
                            'name':
                                m.get('metadata').get(
                                    ...
                                    '-'.join([
                                        i for i in [
                                            recursive_get(m, f'metadata#labels#{await self.labels_name}', sep='#'),
                                            recursive_get(m, f'metadata#labels#{await self.labels_instance}', sep='#'),
                                            cheap_hash(recursive_get(m, f'metadata#labels#{await self.labels_domain_name}', sep='#')),
                                            recursive_get(m, f'metadata#labels#{await self.labels_component}', sep='#')
                                        ] if i
                                    ])
                                )
                        }
                ...
                [
                    ...
                    for r in yaml.load_all(
                        self.jinja.render(
                            ...
                            name=recursive_get(x, f'metadata#labels#{await self.labels_name}', sep='#'),
                            instance=recursive_get(x, f'metadata#labels#{await self.labels_instance}', sep='#'),
                            domain_name=recursive_get(x, f'metadata#labels#{await self.labels_domain_name}', sep='#'),
                            component=recursive_get(x, f'metadata#labels#{await self.labels_component}', sep='#'),
                            managed_by=recursive_get(x, f'metadata#labels#{await self.labels_managed_by}', sep='#'),
            ...
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-renders.yaml'), 'w') as outfile:
    ...
    async def kinds(self):
        return [
            (await self.parent_kind),
            *(await self.monitored_kinds)
        ]
    ...
    async def monitored_kinds(self):
        return [
            k['kind']
            for k
            in recursive_get(await self.template, 'spec.kinds.monitored')
        ]
    ...
    async def template(self):
        if '_template' not in self.__dict__ or self._template is None:
            ...
            self._template = [
                t
                for t
                in await self.get_items("AlfaTemplate")
                if t['metadata']['name'] == self.name
            ]
            if self.config.debug_path:
                with open(os.path.join(self.config.debug_path, f'alfatemplate-{self.name}-template.yaml'), 'w') as outfile:
